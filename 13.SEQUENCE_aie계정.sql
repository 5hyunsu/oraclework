...
/*
    ...
    [CACHE | NOCACHE]    ->캐시 메모리 할당(기본값 CACHE20- 노캐시가 아님) 잘쓰지 않음
    ...
    *캐시 메모리 : 프린터 전송시 캐시메모리 저장, 중간정도 속도
    ...
    이름은 맘대로 바꿀 수 있다. 대체적으로 보자마자 알게끔 하기 위해 
    아래와 같이 이름을 만든다. 
    테이블명: TB_시퀀스이름
    뷰 : VW_   VM
    시퀀스명 : SEQ_
...
MAXVALUE 410 
NOCYCLE --순환 없이 , 넘어가면 오류 뜬다. 안써도 된다. 안쓰면 기본값
NOCACHE;
...
SELECT SEQ_EMPNO.CURRVAL FROM DUAL; --400  한번 가져와야 쓸 수 있다.
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; --405
SELECT SEQ_EMPNO.CURRVAL FROM DUAL; --410
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 지정한 MAX값 초과해서 오류
...
--다음값 하고 같은가 같지 않은가? 하면 할때마다 들어가지 않았는데
--NEXTVAL는 증가한다. 
--300이 들어가저 있으면 NEXTVAL를 쓰고  CURRAL을 300을 쓰겠다. 
/*
    3. 시퀀스 구조 변경 
    ALTER SEQUENCE 시퀀스명
    [INCREMENT BY 숫자]
